...
def best_action_q(state, zobrist_code, last_move, alpha, beta, player, level):
    ...
    interested_n_moves, limited_move = find_interesting_moves(state, move_interest_values, player, level_max_n[level], verbose=False)
...
@numba.jit(nopython=True, nogil=True)
def find_interesting_moves(state, move_interest_values, player, n_moves, verbose=False):
    ...
    interested_n_moves = np.zeros(n_moves*2, dtype=np.int32).reshape(n_moves, 2)
    ...
    if force_to_block is True or exist_will_win_move is True:
        ...
        high_interest_idx = np.argsort(move_interest_values.ravel())[-n_moves:][::-1]
