...
def best_action_q(state, last_move, move_interest, alpha, beta, maximizing_player, level):
    ...
    #        if i_will_win(my_stones, opponent_stones, current_move):
    #            return current_move, 1.0
    #        elif i_will_win(opponent_stones, my_stones, current_move):
    #            local_interest[current_move] = 0.99
    #else:
    #    for current_move in possible_moves:
    #        if i_will_win(opponent_stones, my_stones, current_move):
    #            return current_move, 0.0
    #        elif i_will_win(my_stones, opponent_stones, current_move):
    #            local_interest[current_move] = 0.99

    #threat_moves = find_threat_positions(state)
    #local_interest[zip(*threat_moves)] = 0.99

    adjust_threat_interests(possible_moves, state, local_interest)
...
@numba.jit(nopython=True, nogil=True)
def adjust_threat_interests(possible_moves, state, local_interest):
    my_stones, opponent_stones = state
...
def U_stone(state, last_move, move_interest, alpha, beta, maximizing_player, level):
    ...
    #        result = 0.5 * (this_U + next_U)
        ...
        result = estimate_U(state, maximizing_player)
...
@numba.jit(nopython=True, nogil=True)
def estimate_U(state, maximizing_player):
    my_stones, opponent_stones = state
    board = np.zeros(board_size*board_size, dtype=np.int32).reshape(board_size, board_size)
