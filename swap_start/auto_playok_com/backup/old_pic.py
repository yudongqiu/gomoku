...
def read_game_state(scnshot, image=None):
    if image == None:
...
def swap_waiting(scnshot, image=None):
...
def choose_swap_start(scnshot, strategy):
    ...
    swap_start = 0
    img1 = scnshot.capture() # save an old image without the white bar
    while True:
        time.sleep(0.5)
        img = scnshot.capture()
        swap_start = swap_waiting(scnshot, image=img)
        if swap_start > 0:
            break
        else:
            img1 = img
    ...
    if swap_start == 1: # if i'm first
        ...
        time_spent += swap_choose_side(scnshot, strategy, image=img1)
    return time_spent
...
def check_cursor_playing(scnshot):
    ...
    pyautogui.moveTo(x1, y1+shift*2)
    time.sleep(0.2)
    orig_color = scnshot.capture().getpixel(empty_pos)
    pyautogui.moveTo(x1+c*shift, y1)
    time.sleep(0.3)
...
def swap_choose_side(scnshot, strategy, image=None):
    t_start = time.time()
    x1, y1, x2, y2 = scnshot.border
    w, h = scnshot.width, scnshot.height
    state = read_game_state(scnshot, image=image)
    print_state(state)
