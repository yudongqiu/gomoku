...
board_size = 15
estimate_level = 1
t_random = 0.05 # controls how random the bonus for level=0
show_q = False
...
def strategy(state):
    ...
    if i_win(state, best_move, 1):
        ...
        new_u = 0.0
        game_result = 'draw'
    if game_result and strategy.started_from_beginning is True:
        ...
        # check the win rate of first step, determine if I'm surprised
        if game_result == 'win' and new_u < -0.2:
            strategy.surprised = True
            print(f"Got a surprise win because the initial winrate {new_u} < -0.2")
        elif game_result == 'lose' and new_u > 0.2:
            strategy.surprised = True
            print(f"Got a surprise lose because the initial winrate {new_u} > 0.2")
    elif best_q != None:
...
def best_action_q(state, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    for this_move in interested_moves:
        ...
        state[this_move] = 0
        try:
            if learndata[this_state_id][1] > max_q:
                max_q = learndata[this_state_id][1]
                best_move = this_move
        except KeyError:
...
def tf_predict_u(state, empty_spots_left, last_move, player):
    ...
    # find the known moves among interested_moves
    tf_moves, move_state_ids = [], [] # all unknown moves will be evaluated by tf_evaluate_max_u
    max_q = -1.0
    ...
    if n_tf > 0:
        ...
        for state_id, y in zip(move_state_ids, predict_y):
            tf_predict_u.cache[state_id] = y
...
def initialize():
    ...
    if not hasattr(tf_predict_u, 'all_interest_states'):
        tf_predict_u.all_interest_states = np.zeros(board_size**4 * 3, dtype=np.float16).reshape(board_size**2, board_size, board_size, 3)
