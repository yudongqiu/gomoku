...
class DNNModel(tf.keras.Model):
    ...
    def fit(self, *args, **kwargs):
        self.training = True
        callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, min_delta=0.0005)
        super().fit(*args, **kwargs, callbacks=[callback])
        self.training = False
...
def get_new_model():
    model = DNNModel(n_stages=20, filters=169, kernel_size=3)
