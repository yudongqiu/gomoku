...
class DNNModel(tf.keras.Model):
    ...
    def __init__(self, n_stages=4, filters=256, kernel_size=3):
        ...
        self.layer_conv0 = layers.Conv2D(self.filters, self.kernel_size, padding='valid')
        ...
        # separate output activation to be float32
        self.layer_res_dense = layers.Dense(1, name='dense_logits')
        self.layer_res_atv = layers.Activation('tanh', dtype='float32', name='predictions')
    ...
    def call(self, inputs, training=False):
        ...
        x = self.layer_res_dense(x)
        res = self.layer_res_atv(x)
        return res

    def fit(self, *args, **kwargs):
        callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, min_delta=0.0005)
        super().fit(*args, **kwargs, callbacks=[callback], batch_size=256)
...
def get_new_model():
    ...
    model = DNNModel(n_stages=5, filters=168, kernel_size=3)
    optimizer = tf.optimizers.SGD(learning_rate=0.003)
...
def load_existing_model(path):
    model = get_new_model()
    model.train_on_batch(np.zeros((1,15,15,3)), np.zeros(1))
