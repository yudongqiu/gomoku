...
level_max_n = [20, 20, 10, 10, 8, 8, 6, 6, 4, 4, 4, 4, 4, 4, 4]
...
def best_action_q(state, zobrist_code, last_move, alpha, beta, player, level):
    ...
    # boost the interests of closer moves
    #boost_dist = 3
    #r, c = last_move
    ...
    if limited_move:
        ...
        q = Q_stone(state, zobrist_code, current_move, alpha, beta, player, level)
        return current_move, q

    #best_move = (-1,-1) # admit defeat if all moves have 0 win rate
    best_move = (interested_n_moves[0,0], interested_n_moves[0,1]) # continue to play even I'm losing

    if player == 1:
        max_q = 0.0
        for current_move in interested_n_moves:
            current_move = (current_move[0], current_move[1])
            q = Q_stone(state, zobrist_code, current_move, alpha, beta, player, level+1)
            if q > alpha: alpha = q
            if q > max_q:
                if level == 0:
...
def cached_interesting_moves(state, zobrist_code, player, n_moves):
    try:
        result = cached_interesting_moves.cache[zobrist_code]
    except KeyError:
        move_interest_values = -np.ones(board_size**2, dtype=np.float32).reshape(board_size,board_size)
        cached_interesting_moves.cache[zobrist_code] = result = find_interesting_moves(state, move_interest_values, player) 
    n_moves = min(n_moves, np.sum(state==0))
    interested_n_moves, limited_move = result
    return interested_n_moves[:n_moves], limited_move


@numba.jit(nopython=True, nogil=True)
def find_interesting_moves(state, move_interest_values, player, verbose=False):
    ...
    output:
    -------
    interested_n_moves: numpy.array n_moves x 2
    limited_move: True if only interested_n_moves[0] is valid
    """
    # check how many empty spots are left
    force_to_block = False
    exist_will_win_move = False
    directions = ((1,1), (1,0), (0,1), (1,-1))

    interested_n_moves = np.zeros(board_size*board_size*2, dtype=np.int32).reshape(board_size*board_size, 2)
            ...
            for dr, dc in directions:
                ...
                if force_to_block is False and exist_will_win_move is False:
                    ...
                    if forward_my_open is True:
                        interest_value += (my_line_length - 1) ** 3
                    if backward_my_open is True:
                        interest_value += (my_line_length_back - 1) ** 3
                    if forward_opponent_open is True:
                        interest_value += (opponent_line_length - 1) ** 3
    ...
    if force_to_block is True or exist_will_win_move is True:
        ...
        high_interest_idx = np.argsort(move_interest_values.ravel())[::-1]
        interested_n_moves[:,0] = high_interest_idx // board_size
        interested_n_moves[:,1] = high_interest_idx % board_size
        limited_move = False

    if verbose is True:
        if limited_move is True:
            print(interested_n_moves[0,0], interested_n_moves[0,1], 'Only one')
        else:
            for i in range(10):
                print(interested_n_moves[i,0], interested_n_moves[i,1], move_interest_values.ravel()[high_interest_idx[i]])

    return interested_n_moves, limited_move


def Q_stone(state, zobrist_code, current_move, alpha, beta, player, level):
    # update the state
    ...
    result = U_stone(state, new_zobrist_code, current_move, alpha, beta, player, level)
    # revert the changes for the state
    state[current_move] = 0
    return result

def U_stone(state, zobrist_code, last_move, alpha, beta, player, level):
    try:
    ...
    if i_will_win(state, last_move, player):
        ...
        best_move, best_q = best_action_q(state, zobrist_code, last_move, alpha, beta, -player, level)
        result = best_q
...
def initialize():
    ...
    if not hasattr(cached_interesting_moves, 'cache'):
        cached_interesting_moves.cache = dict()
