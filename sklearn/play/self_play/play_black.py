...
def U_stone(state, zobrist_code, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    elif level >= estimate_level and player == 1: # only work after my stone is placed
        result = U_stone.estimate(state.reshape(1,-1))[0]
    else:
...
def initialize():
    ...
    if not hasattr(strategy, 'learndata'):
        ...
        if os.path.exists(filename):
            ...
            raise RuntimeError("%s is not found!" %filename)
        from sklearn.tree import DecisionTreeRegressor
        x, y, n = zip(*strategy.learndata.values())
        train_x = np.array(x).reshape(len(x), -1)
        train_y = np.array(y)
        regr = DecisionTreeRegressor(max_depth=30)
        regr.fit(train_x, train_y, sample_weight=n)
        print("Trainning finished. Score: %f" %regr.score(train_x, train_y))
        U_stone.estimate = regr.predict
