...
def strategy(state):
    ...
    if last_move is None: # if it's the first move of the game
        ...
        # clear the U cache
        U_stone.cache = strategy.cachehigh.copy()
    ...
    if best_q == 1.0:
        if strategy.started_from_beginning is True:
            # i'm winning! Let me boost the win rate all the history states
            reduce_value = 1.0
            for prev_state_zobrist_code in strategy.hist_states[::-1]:
                u = strategy.cachehigh[prev_state_zobrist_code]
                new_u = u + reduce_value * learning_rate * (1.0 - u)
                strategy.cachehigh[prev_state_zobrist_code]= new_u
                reduce_value *= reduce_factor
            print("Increased win rate of %d states" % len(strategy.hist_states))
        strategy.started_from_beginning = False
    elif best_q == 0.0:
        if strategy.started_from_beginning is True:
            # i'm losing! Let me boost the win rate all the history states
            reduce_value = 1.0
            for prev_state_zobrist_code in strategy.hist_states[::-1]:
                u = strategy.cachehigh[prev_state_zobrist_code]
                new_u = u + reduce_value * learning_rate * (0.0 - u)
                strategy.cachehigh[prev_state_zobrist_code]= new_u
                reduce_value *= reduce_factor
                print("white:Decreased win rate of %d from %f to %f"%(prev_state_zobrist_code, u,new_u))
            print("Decreased win rate of %d states" % len(strategy.hist_states))
        strategy.started_from_beginning = False
    elif empty_spots_left > 100:
        # record the history states
        strategy.hist_states.append(strategy.zobrist_code)
    # return the best move
    print("white: best_q = %f"%best_q)
    return (best_move[0]+1, best_move[1]+1)
...
def U_stone(state, zobrist_code, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    if i_will_win(state, last_move, player):
        ...
        result = estimate_U(state, player)# - 0.0005 + np.random.random() * 0.001
    else:
...
def initialize():
    ...
    if not hasattr(strategy, 'cachehigh'):
        if strategy.playing == 0:
            filename = 'black.cachehigh'
        else:
            filename = 'white.cachehigh'
...
def finish():
    ...
    print("Successfully saved %d win rates to %s"%(len(strategy.cachehigh), filename))
