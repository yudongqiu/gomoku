...
def strategy(state):
    ...
    if game_finished and strategy.started_from_beginning is True:
        ...
        for prev_state_zobrist_code in strategy.hist_states[::-1]:
            x, u, n_visited = strategy.learndata[prev_state_zobrist_code]
            n_visited += 1
            new_u = u + discount * (new_u - u) /  n_visited # this is the learning rate
            strategy.learndata[prev_state_zobrist_code] = x, new_u, n_visited
...
def U_stone(state, zobrist_code, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    if i_will_win(state, last_move, player):
        ...
        try:
            result = strategy.learndata[zobrist_code][1]
        except:
...
def estimate_U(state, player):
    ...
    my_max_n = 0
    opponent_max_n = 0
    u_mat = np.zeros(board_size**2, dtype=np.float32).reshape(board_size,board_size)
    ...
    if u > 0:
        result = 1.0 - 0.5 * np.exp(-u**3 * 0.000001)
    else:
        result = 0.5 * np.exp(u**3 * 0.000001)
    return result
