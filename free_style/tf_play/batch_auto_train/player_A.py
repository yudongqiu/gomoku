...
def strategy(state):
    ...
    if game_finished and strategy.started_from_beginning is True:
        ...
        for prev_state_zobrist_code in strategy.hist_states[::-1]:
            state , u, n_visited = strategy.learndata[prev_state_zobrist_code]
            n_visited += 1
            new_u = u + discount * (new_u - u) /  n_visited # this is the learning rate
            strategy.learndata[prev_state_zobrist_code] = (state, new_u, n_visited)
...
def tf_predict_u(state, zobrist_code, empty_spots_left, last_move, player):
    ...
    #draw_state(tf_state)
    #for s, y in zip(interested_moves, predict_y):
    #    print(s,y)
