...
def strategy(state):
    ...
    if strategy.zobrist_code not in strategy.learndata:
    ...
    if game_finished and strategy.started_from_beginning is True:
        ...
        for prev_state_zobrist_code in strategy.hist_states[::-1]:
            ...
            n_visited += 1
            new_u = u + discount * (new_u - u) /  n_visited # this is the learning rate
...
def best_action_q(state, zobrist_code, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    if player == 1:
        ...
        for current_move in interested_moves:
            ...
            #    n_visited = 0 if next_zobrist_code not in strategy.learndata else strategy.learndata[next_zobrist_code][2]
            #    bonus_q = 0.1 / (n_visited + 1)
            #    if q + bonus_q > max_q:
            #        max_q = q + bonus_q
...
def U_stone(state, zobrist_code, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    if i_will_win(state, last_move, player):
        ...
        result = tf_predict_u(state,player)
        #print(last_move, result)
    else:
    ...
    #if level == 1:
    #    if zobrist_code not in strategy.learndata:
    #        strategy.learndata[zobrist_code] = state, result
    return result

def tf_predict_u(state, player):
    tf_state = tf_predict_u.tf_state
    if player == 1: # if I just played
        tf_state[0] = (state == 1)
        tf_state[1] = (state == -1)
        tf_state[2] = (state == 0)
        tf_state[4] = 1 if strategy.playing == 0 else 0
        result = tf_predict_u.model.predict([tf_state])[0][0]
    elif player == -1: # If opponent just played
        tf_state[0] = (state == -1)
        tf_state[1] = (state == 1)
        tf_state[2] = (state == 0)
        tf_state[4] = 1 if strategy.playing == 1 else 0
        result = 1.0 - tf_predict_u.model.predict([tf_state])[0][0]
    return result
...
def estimate_U(state, player):
    ...
    if u > 0:
        result = 1.0 - 0.5 * np.exp(-u**3 * 0.0000001)
    else:
        result = 0.5 * np.exp(u**3 * 0.0000001)
    return result
...
def initialize():
    ...
    if not hasattr(tf_predict_u, 'tf_state'):
        tf_predict_u.tf_state = np.zeros(5 * board_size**2, dtype=np.int32).reshape(5, board_size, board_size)
...
def finish():
    ...
    #
    #filename = color + '.learndata'
    x, y, n = zip(*strategy.learndata.values())
    x = np.array(x)
    y = np.array(y).reshape(-1,1)
    n_data = len(x)
    train_X = np.empty(n_data*5*15*15, dtype=np.int32).reshape(n_data, 5, 15, 15)
    train_X[:, 0, :, :] = (x == 1) # first plane indicates my stones
    train_X[:, 1, :, :] = (x == -1) # first plane indicates opponent_stones
    train_X[:, 2, :, :] = (x == 0) # third plane indicates empty spots
    train_X[:, 3, :, :] = 1 # fourth plane is all 1
    train_X[:, 4, :, :] = 1 if strategy.playing == 0 else 1 # 5th plane indicates if i'm black
    train_Y = y
    model = tf_predict_u.model
    model.fit(train_X, train_Y, n_epoch=10, validation_set=0.1, show_metric=True)
    model.save("tf_model")
    print("Model saved to tf_model!")

    #pickle.dump(strategy.learndata, open(filename, 'wb'))
    #print("Successfully saved %d learndata to %s"%(len(strategy.learndata), filename))
