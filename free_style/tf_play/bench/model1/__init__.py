...
def best_action_q(state, zobrist_code, empty_spots_left, last_move, alpha, beta, player, level):
    ...
    if player == 1:
        ...
        for current_move in interested_moves:
            ...
            if level == 0 and q > 0:
                bonus_q = abs(np.random.normal(0, t_random))
                if q + bonus_q > max_q:
                    max_q = q + bonus_q
...
def initialize():
    ...
    if not hasattr(tf_predict_u, 'model'):
        import construct_dnn1
        model = construct_dnn1.construct_dnn()
